...
export class RvsSpreadsheetFile implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'RVS Spreadsheet File',
		name: 'rvsSpreadsheetFile',
		icon: 'fa:table',
		group: ['transform'],
		version: 1,
		description: 'Reads and writes data from a spreadsheet file like CSV, XLS, ODS, etc',
		defaults: {
			name: 'RVS Spreadsheet File',
			color: '#2244FF',
		},
		inputs: ['main'],
		outputs: ['main'],
		properties: [
			{
				...
				options: [
					{
						name: 'Read Sheet Names',
						value: 'readSheetNames',
						description: 'Reads sheet names from a spreadsheet file',
						action: 'Read sheet names from a spreadsheet file',
					},
					{
						name: 'Read From File',
						value: 'fromFile',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		} else if (operation === 'readSheetNames') {
			for (let i = 0; i < items.length; i++) {
				try {
					const item = items[i];
