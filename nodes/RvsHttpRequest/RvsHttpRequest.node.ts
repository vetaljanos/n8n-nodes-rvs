...
import type {IBinaryKeyData, IDataObject, INodeExecutionData, INodeType, INodeTypeDescription,} from 'n8n-workflow';
...
export class RvsHttpRequest implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'RVS HTTP Request',
		name: 'rvsHttpRequest',
		icon: 'fa:at',
		group: ['output'],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			...
			} else requestOptions.json = bodyContentType !== 'raw';
			...
			if (bodyContentType === 'raw') {
				...
				requestOptions.headers['Content-Type'] = this.getNodeParameter('rawContentType', itemIndex) as string;
			}
